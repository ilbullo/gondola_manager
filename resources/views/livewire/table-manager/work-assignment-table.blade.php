...
<div wire:refresh class="h-full flex flex-col">
    <!--
    <div id="loading-modal" {{-- Usa 'hidden' di default e rimuovi 'hidden' quando Livewire è in caricamento --}} wire:loading.class.remove="hidden" {{-- Ora puoi lasciare 'flex' e le classi di centratura, perché 'hidden' ha la precedenza --}}
        class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] hidden" role="status"
        aria-live="polite">
            ...
            <div class="overflow-auto flex-1 w-full">
                <div class="min-w-[1200px] w-full">
                    <table class="w-full border-collapse">
                        ...
                        <caption class="sr-only">Tabella di assegnazione lavori. Le righe rappresentano le licenze, le
                            colonne gli slot orari.</caption>

                        {{-- Header --}}
                        <thead class="bg-gray-50 sticky top-0 z-20 shadow-sm">
                            <tr>
                                <th scope="col"
                                    class="p-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider sticky left-0 bg-gray-50 border-b border-r border-gray-200 w-32 z-30">
                                    Licenza
                                </th>
                                @for ($i = 1; $i <= config('constants.matrix.total_slots'); $i++)
                                    <th scope="col"
                                        class="p-2 text-center text-xs font-bold text-gray-600 uppercase border-b border-gray-200 min-w-[3rem]">
                                        {{ $i }}
                                    </th>
                                @endfor
                            </tr>
                                            ...
                                            @if ($hasWork)
                                                <span class="text-xs font-bold text-gray-900 block leading-tight">
                                                    ...
                                                    @if ($workData['excluded'])
                                                        <span
                                                            class="inline-block px-1.5 py-0.5 mt-1 text-[9px] font-bold rounded-full bg-red-100 text-red-700">
                                                            F
                                                        </span>
                                                    @endif
                                                    @if ($workData['shared_from_first'])
                                                        <span
                                                            class="inline-block px-1.5 py-0.5 mt-1 text-[9px] font-bold rounded-full bg-emerald-100 text-emerald-700">
                                                            R
                                                        </span>
                                                    @endif
                                                </span>
                                            @else
                                                <div role="gridcell" aria-label="Cella vuota"
